...
- 브랜드 소개, 핵심 가치
- 파트너십 안내, CTA 섹션
- **번역 키**: 18개 (히어로, 설명, 버튼, 가치, 파트너십)

```typescript
<h1>{t('about.hero.title')}</h1>
<p>{t('about.hero.description')}</p>
<Button>{t('about.hero.ctaButton')}</Button>
```

#### 4. **상품 상세 페이지** (`src/app/products/[id]/page.tsx`)
- 상품 정보, 가격, 재고
- 장바구니/위시리스트 버튼
- 인기/할인 배지
- **번역 키**: 28개 (제품 정보, 버튼, 알림 메시지)

```typescript
<Badge>{t('products.popular')}</Badge>
<Button>{t('productDetail.addToCart')}</Button>
alert(t('cart.addSuccess', { name: product.name, count: quantity }));
```

#### 5. **공지사항 페이지** (`src/app/notices/page.tsx`)
- 공지사항 목록, 검색, 필터
- 카테고리별 분류
- **번역 키**: 8개 (제목, 검색, 필터, 페이지네이션)

```typescript
<h1>{t('notices.title')}</h1>
<Input placeholder={t('notices.searchPlaceholder')} />
<SelectItem value="general">{t('notices.general')}</SelectItem>
```

#### 6. **문의 페이지** (`src/app/contact/page.tsx`)
- 문의 양식, 필드 레이블
- 문의 유형 선택
- **번역 키**: 12개 (제목, 폼 필드, 버튼, 메시지)
...
#### 7. **장바구니 페이지** (`src/app/cart/page.tsx`)
- 장바구니 아이템 관리
- 전체 선택, 가격 계산
- **번역 키**: 장바구니 전용
...
#### 8. **체크아웃 페이지** (`src/app/checkout/page.tsx`)
- 배송 정보 입력
- 결제 수단 선택
- 약관 동의
- **번역 키**: 체크아웃 전용
...
```typescript
<h1>{t('content.blog.title')}</h1>
<Input placeholder={t('content.blog.searchPlaceholder')} />
<Badge>{t('content.blog.featured')}</Badge>
```
...
```typescript
<h1>{t('content.video.title')}</h1>
<p>{t('content.video.loading')}</p>
```

### 📊 통계 요약
- **총 번역 키**: 358개
- **적용된 페이지**: 10개 주요 사용자 페이지
- **지원 언어**: 3개 (한국어, 영어, 중국어)
- **번역 파일 크기**: 43.2 KB (Gzip: ~12-15 KB)
...
### 2025-10-21 (최신 업데이트)
- ✅ **핵심 사용자 페이지 다국어 시스템 완전 구축 완료**
- ✅ About 페이지 다국어 적용 (18개 번역 키)
- ✅ 상품 상세 페이지 다국어 적용 (28개 번역 키)
- ✅ 공지사항 페이지 다국어 적용 (8개 번역 키)
- ✅ 문의 페이지 다국어 적용 (12개 번역 키)
- ✅ 콘텐츠 페이지 다국어 적용 - 블로그/비디오 (11개 번역 키)
- ✅ 총 358개 번역 키 완전 번역 (ko, en, zh)
- ✅ 자동 검증 스크립트 구축 (`scripts/verify-i18n.js`)
- ✅ 상세 보고서 생성기 구축 (`scripts/generate-i18n-report.js`)
- ✅ 전체 시스템 검증 완료 (번역 파일 일관성 100%)
...
#### 1. `scripts/verify-i18n.js`
다국어 시스템의 전체 일관성을 자동으로 검증합니다.

**기능:**
- ✅ 번역 파일 구문 검증 (JSON 파싱 오류 확인)
- ✅ 번역 키 일관성 검증 (모든 언어에 동일한 키가 있는지)
- ✅ 코드에서 사용된 키 추출
- ✅ 누락된 번역 키 감지
- ✅ 사용되지 않는 번역 키 감지
- ✅ 빈 번역 값 검증
...
**출력 예시:**
```
📝 Step 1: 번역 파일 로드 및 구문 검증
✓ ko.json 로드 성공 (14056 bytes)
✓ en.json 로드 성공 (17564 bytes)
✓ zh.json 로드 성공 (12668 bytes)
...
🔍 Step 3: 코드에서 사용된 번역 키 추출
ℹ 코드에서 538개의 번역 키 사용 발견

✅ Step 4: 사용된 키 검증
✓ 모든 사용된 키가 번역 파일에 존재합니다!
...
#### 2. `scripts/generate-i18n-report.js`
상세한 다국어 시스템 보고서를 생성합니다.
...
- [ ] 모든 언어 파일에 동일한 키가 있는가?
- [ ] 코드에서 사용된 모든 키가 번역 파일에 있는가?
- [ ] 번역 값에 빈 문자열이 없는가?
- [ ] 빌드가 성공하는가?
- [ ] 언어 전환이 정상 작동하는가?
- [ ] 동적 파라미터가 올바르게 치환되는가?
- [ ] localStorage 저장이 정상 작동하는가?
...
#### Gzip 압축
Next.js는 자동으로 JSON 파일을 압축합니다.
- 원본 크기: ~43 KB
- Gzip 후: ~12-15 KB (70% 압축률)

#### 동적 로딩
`LanguageContext`는 필요한 언어 파일만 동적으로 로드합니다.
```typescript
...
1. **번역 키 추가**
   ```bash
   # 1. 3개 언어 파일에 키 추가
   # src/locales/ko.json, en.json, zh.json
...
1. **설정 파일 수정** (`src/i18n/config.ts`)
   ```typescript
   export const locales = ['ko', 'en', 'zh', 'ja'] as const; // 'ja' 추가
...
```typescript
// 1. 키가 올바른지 확인
console.log(t('cart.title')); // 'cart.title'이 반환되면 키가 없는 것

// 2. 언어 파일 로드 확인
console.log(messages); // 빈 객체면 파일 로드 실패
...
```typescript
// JSON에서 확인
"totalItems": "총 {count}개의 상품" // ✅ 올바름
"totalItems": "총 ${count}개의 상품" // ❌ 잘못됨

// 코드에서 확인
t('cart.totalItems', { count: 5 }) // ✅ 올바름
t('cart.totalItems', { total: 5 })  // ❌ 키 이름 불일치
```
