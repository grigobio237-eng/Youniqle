...
export default function SchedulePage() {
  ...
  const isCoach = (session?.user as any)?.footballRole === 'coach';
  ...
  const fetchEvents = async () => {
    try {
      ...
      const res = await fetch('/api/football/announcements?type=schedule');
      if (res.ok) {
  ...
  const handleCreateSchedule = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/football/announcements', {
        method: 'POST',
  ...
  const handleDeleteSchedule = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/football/announcements?id=${id}`, {
        method: 'DELETE',
      });
