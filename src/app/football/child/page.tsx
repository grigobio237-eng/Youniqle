...
export default function ChildDashboardPage() {
  ...
  const fetchChildData = async () => {
    try {
      const res = await fetch('/api/football/guardian?days=14');
      if (res.ok) {
  ...
  const handleLink = async () => {
    ...
    setLinkResult(null);
    try {
      const res = await fetch('/api/football/guardian', {
        method: 'POST',
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-pink-50 to-background p-4 pb-24">
        <div className="max-w-lg mx-auto space-y-6 pt-4 md:pt-24">
          <Link href="/football/mypage" className="text-slate hover:text-obsidian inline-flex items-center gap-1 text-sm">
  ...
  // 자녀 연결 완료 — 데이터 열람
  const { child, todayCheck, checks, stats } = data;
          ...
          <CardContent className="p-6 space-y-4">
            ...
            {todayCheck ? (
              <div className="text-center space-y-3">
                ...
                <p className="text-sm text-slate">웰니스 점수 (5점 만점)</p>
                <div className="grid grid-cols-5 gap-2 text-center">
                  {[
                    ...
                    <div key={item.label} className={`py-2 rounded-lg text-xs font-bold transition-all ${
                      ...
                      {item.subText && <p className="text-[8px] font-black opacity-80 mt-0.5">{item.subText}</p>}
                    </div>
                  ))}
                </div>
