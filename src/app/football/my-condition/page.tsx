...
export default function MyConditionPage() {
  ...
  const fetchCondition = async () => {
    try {
      const res = await fetch('/api/football/wellness?view=my&days=28');
      if (res.ok) {
        ...
        <Card className="max-w-md w-full rounded-[32px] border-none shadow-2xl">
          <CardContent className="p-10 text-center space-y-6">
            ...
            <Button asChild className="w-full h-12 rounded-2xl font-black bg-green-600 hover:bg-green-700">
              <Link href="/football/wellness">오늘의 컨디션 체크하기</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="flex items-center justify-between">
              ...
              {todayCheck ? (
                ...
                <Button asChild size="sm" className="bg-green-600 hover:bg-green-700 rounded-xl h-8 text-xs">
                  <Link href="/football/wellness">체크하기 →</Link>
                </Button>
              )}
            </div>
            {todayCheck ? (
              <>
                <div className="overflow-x-auto pb-2 custom-scrollbar">
                  ...
                  {[
                    { label: '수면', value: todayCheck.sleep, emoji: '🌙', subText: todayCheck.sleepDuration ? `${todayCheck.sleepDuration}시간` : undefined },
                    { label: '통증', value: todayCheck.soreness, emoji: '💪' },
                    { label: '피로', value: todayCheck.fatigue, emoji: '⚡' },
                    { label: '스트레스', value: todayCheck.stress, emoji: '🧠' },
                    { label: '기분', value: todayCheck.mood, emoji: '😊' },
                  ].map((item) => (
                    <div key={item.label} className="space-y-1">
                      <span className="text-lg">{item.emoji}</span>
                      <div className={`text-xl font-black ${
                        item.value >= 4 ? 'text-green-600' : item.value >= 3 ? 'text-yellow-600' : 'text-red-600'
                      }`}>{item.value}</div>
                      <p className="text-[10px] text-slate">{item.label}</p>
                      {item.subText && <p className="text-[9px] font-bold text-slate/85 mt-0.5">{item.subText}</p>}
                    </div>
                  ))}
                  </div>
                </div>
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="text-center bg-white/30 rounded-xl p-3">
                  <p className="text-xs font-bold opacity-70">급성 부하 (7일)</p>
                  <p className="text-lg font-black">{acwr.acuteLoad} AU</p>
                </div>
                <div className="text-center bg-white/30 rounded-xl p-3">
                  <p className="text-xs font-bold opacity-70">만성 부하 (28일)</p>
                  <p className="text-lg font-black">{acwr.chronicLoad} AU/주</p>
                </div>
              </div>
              {acwr.zone === 'danger' && (
                <div className="flex items-center gap-2 bg-red-200 rounded-xl p-3 text-sm font-bold">
                  <AlertTriangle className="w-4 h-4 flex-shrink-0" />
                  부상 위험이 높습니다. 훈련 강도를 줄이는 것이 좋습니다.
                </div>
              )}
            </CardContent>
          </Card>
