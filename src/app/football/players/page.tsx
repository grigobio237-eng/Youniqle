...
function PlayersContent() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (playerId) {
        // 특정 선수 상세 (28일 데이터)
        const res = await fetch(`/api/football/wellness?view=my&days=28&playerId=${playerId}`);
        if (res.ok) {
        ...
        // 선수 명단
        const res = await fetch(`/api/football/wellness?view=team&teamId=${teamId}`);
        if (res.ok) {
          const data = await res.json();
          setSquad(data.squad || []);
        }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-green-50 to-background p-4 pb-24">
        <div className="max-w-lg mx-auto space-y-6 pt-4 md:pt-24">
          <div>
            <Link href="/football/players" className="text-slate hover:text-obsidian inline-flex items-center gap-1 text-sm mb-1">
              <ArrowLeft className="w-4 h-4" /> 선수 명단
            </Link>
            <h1 className="text-2xl font-black text-obsidian flex items-center gap-2">
              <User className="w-6 h-6" /> 선수 상세 기록
            </h1>
          </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-lg font-black ${
                      check.wellnessScore >= 4 ? 'text-green-600' :
                      check.wellnessScore >= 3 ? 'text-yellow-600' : 'text-red-600'
                    }`}>{check.wellnessScore}</span>
                  </div>
                  <div className="grid grid-cols-5 gap-1 text-center text-[10px]">
                    {[
                      { l: '수면', v: check.sleep, sub: check.sleepDuration ? `(${check.sleepDuration}h)` : '' },
                      { l: '통증', v: check.soreness },
                      ...
                      <div key={item.l} className={`py-1 rounded-lg font-bold flex flex-col items-center justify-center ${
                        ...
                        {item.sub && <span className="text-[8px] opacity-75 font-black mt-0.5">{item.sub}</span>}
                      </div>
                    ))}
                  </div>
                  {check.sessionLoad && (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-background p-4 pb-24">
      <div className="max-w-lg mx-auto space-y-6 pt-4 md:pt-24">
        <div>
          <Link href="/football/mypage" className="text-slate hover:text-obsidian inline-flex items-center gap-1 text-sm mb-1">
            <ArrowLeft className="w-4 h-4" /> 클럽하우스 홈
          </Link>
          <h1 className="text-2xl font-black text-obsidian flex items-center gap-2">
            <Users className="w-6 h-6" /> 선수 명단
          </h1>
          {teamInfo && (
            <p className="text-sm text-slate mt-1">{teamInfo.team?.teamName} • 선수 {squad.length}명</p>
          )}
        </div>
        ...
        {squad.length === 0 ? (
          <Card className="rounded-2xl border-none shadow-xl">
            <CardContent className="p-10 text-center space-y-4">
              ...
              <p className="text-slate">아직 선수가 없습니다</p>
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-3">
            {squad.map((player) => {
              ...
              return (
                <Link key={player.memberId} href={`/football/players?id=${player.userId}`}>
