...
const PLAN_FEATURES: Record<string, { icon: string; features: string[] }> = {
  basic: { icon: '🥉', features: ['선수 20명 관리', '웰니스 체크', '신호등 대시보드', '공지사항 관리'] },
  pro: { icon: '🥈', features: ['선수 40명 관리', '베이직 전체', 'ACWR 분석', 'AI 주간 리포트', '보호자 연동'] },
  premium: { icon: '🥇', features: ['선수 100명 관리', '프로 전체', '다중 팀 관리', '영양 분석', '전용 상담'] },
};
...
export default function SubscriptionPage() {
  ...
  const fetchSubscription = async () => {
    try {
      const res = await fetch('/api/football/subscription');
      if (res.ok) {
  ...
  const handlePayment = async (plan: string) => {
    ...
    try {
      const response = await fetch('/api/football/subscription/payment/request', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ plan }),
      });
