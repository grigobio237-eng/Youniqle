...
export default function TeamJoinPage() {
  ...
  // 폼 상태
  const [role, setRole] = useState<'player' | 'guardian'>('player');
  ...
  const fetchTeamInfo = async () => {
    try {
      const res = await fetch(`/api/football/team/join?teamCode=${teamCode}`);
      if (res.ok) {
  ...
  const handleJoin = async () => {
    if (!session) {
      router.push(`/auth/signin?callbackUrl=/football/join/${teamCode}`);
      return;
    }
    ...
    try {
      const res = await fetch('/api/football/team/join', {
        method: 'POST',
          ...
          <CardContent className="p-10 text-center space-y-6">
            ...
            <div className="flex flex-col gap-3 pt-4">
              <Button asChild className="w-full h-14 rounded-2xl font-black bg-primary">
                <Link href="/football/mypage">⚽ 나의 클럽하우스 시작하기</Link>
              </Button>
          ...
          <CardContent className="p-8 space-y-4">
            ...
            <div className="flex items-center gap-6 text-sm text-slate">
              <span className="flex items-center gap-1">
                <Users className="w-4 h-4" /> 선수 {team.memberCount || 0}명
              </span>
              {team.region && (
                <span className="flex items-center gap-1">
              ...
              <span className="flex items-center gap-1">
                <Shield className="w-4 h-4" /> {team.createdBy?.name || '코치'}
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-8 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <p className="font-black text-obsidian mt-1">선수</p>
                ...
                >
                  <span className="text-2xl">👨‍👩‍👦</span>
                  <p className="font-black text-obsidian mt-1">보호자</p>
            ...
            {!session ? (
              <div className="space-y-3">
                ...
                >
                  <Link href={`/auth/signin?callbackUrl=/football/join/${teamCode}`}>
                    로그인 후 합류하기
                  </Link>
                </Button>
              </div>
