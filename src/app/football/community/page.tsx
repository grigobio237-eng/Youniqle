...
interface CommunityPost {
  ...
  authorId: { _id: string; name: string; image?: string; footballRole?: string };
  authorName: string;
  authorImage?: string;
...
export default function CommunityPage() {
  ...
  const fetchPosts = async () => {
    try {
      const res = await fetch(`/api/football/community?category=${filter}`);
      if (res.ok) {
  ...
  const handleCreatePost = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/football/community', {
        method: 'POST',
  ...
  const handleToggleLike = async (postId: string, e: React.MouseEvent) => {
    ...
    try {
      const res = await fetch('/api/football/community/like', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleCreateComment = async (postId: string, e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/football/community/comment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
