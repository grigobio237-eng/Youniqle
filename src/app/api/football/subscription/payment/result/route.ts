...
import FootballSubscription, { PLAN_CONFIG } from '@/models/FootballSubscription';
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (authResultCode !== '0000') {
            ...
            return NextResponse.redirect(`${baseUrl}/football/subscription?error=auth_failed`);
        }
        ...
        if (resultJson.ResultCode !== '3001') { // 3001이 승인 성공 (버전에 따라 다를 수 있으나 보통 3001)
            ...
            return NextResponse.redirect(`${baseUrl}/football/subscription?error=approve_failed`);
        }
        ...
        return NextResponse.redirect(`${baseUrl}/football/subscription?success=true`);
        
    } catch (error) {
        console.error('Subscription Payment Result Error:', error);
        const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000';
        return NextResponse.redirect(`${baseUrl}/football/subscription?error=server_error`);
    }
}
