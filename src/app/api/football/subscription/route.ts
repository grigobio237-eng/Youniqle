...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!membership) {
      return NextResponse.json({ hasSubscription: false, message: '코치 권한이 필요합니다' });
    }

    const subscription = await FootballSubscription.findOne({
    ...
    const team = await FootballTeam.findById(membership.teamId).select('teamName');
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 이미 활성 구독이 있는지 확인
    const existing = await FootballSubscription.findOne({
      teamId: membership.teamId,
      status: { $in: ['trial', 'active'] },
    });
    ...
    const subscription = await FootballSubscription.create({
      userId: session.user.id,
      teamId: membership.teamId,
      plan,
      status: 'trial',
