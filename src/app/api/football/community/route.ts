...
export async function GET(req: NextRequest) {
  try {
    ...
    const posts = await CommunityPost.find(query)
      .populate('authorId', 'name image footballRole')
      .sort({ createdAt: -1 })
      .limit(30);

    return NextResponse.json({ success: true, posts });
  } catch (error: any) {
    console.error('[Football Community GET]', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!membership) {
      return NextResponse.json({ error: '소속된 축구팀이 없습니다. 팀 가입이 필요합니다.' }, { status: 403 });
    }

    // 사용자 정보 상세 조회 (작성자 프로필 이미지 및 이름 동기화용)
    const user = await User.findById(session.user.id);
    const authorName = user?.name || session.user.name || '익명 선수';
    ...
    console.error('[Football Community POST]', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
