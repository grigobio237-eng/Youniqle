...
export async function POST(req: NextRequest) {
  try {
    ...
    // 주석 해제하여 권한 체크 진행 
    // if (!session || session.user?.footballRole !== 'coach') {
    ...
    // 1. 여기서 DB에 선수 상태 업데이트 로직 (목업)
    // await db.collection('football_players').updateOne({ _id: playerId }, { $set: { currentLoad: trainingLoad, lastComment: coachComment } });

    // 2. 부모 알림 트리거 로직 (목업)
    // const parent = await db.collection('users').findOne({ role: 'guardian', linkedPlayerId: playerId });
