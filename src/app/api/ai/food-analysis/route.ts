...
export async function POST(request: NextRequest) {
    try {
        ...
        {
            "foodName": "음식/대상 명칭",
            "summary": "회복 관점에서의 핵심 가치 한 줄 요약 (예: 근육 회복과 피로 해소에 최적화된 식단)",
            "nutritionTable": [
                { "label": "주요성분/항목", "value": "수치/양", "benefit": "회복에 미치는 구체적 이점" },
                { "label": "단백질", "value": "25g", "benefit": "조직 재생 및 면역력 강화" },
                { "label": "비타민C", "value": "항산화", "benefit": "피로 물질 제거 및 염증 완화" }
            ],
            "futureDirection": "향후 섭취 시 주의사항 또는 추천되는 다음 회복 단계 (한 문장 내외)",
            "matchScore": "0~100 사이의 숫자 (사용자 여정 및 데이터와의 궁합)"
        }

        [RULES]
        1. 데이터 구조화: 모든 영양 정보와 분석은 'nutritionTable' 배열에 3~5개의 핵심 항목으로 나누어 담으세요. 서술형 문장('analysis')은 제거하고 'summary'와 'futureDirection'으로 대체합니다.
