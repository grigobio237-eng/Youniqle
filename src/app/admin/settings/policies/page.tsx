  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-obsidian tracking-tight">약관 및 보안 관리</h1>
          <p className="text-foreground/70 mt-1 flex items-center">
            <Settings2 className="w-4 h-4 mr-2" />
            웹 화면에 노출되는 동의서 및 약관을 동적으로 설정하고 버전을 관리합니다.
          </p>
        </div>
        <Button asChild className="bg-secondary hover:bg-secondary font-bold rounded-xl h-12 px-6">
      ...
      <div className="grid grid-cols-1 gap-6">
        <Card className="border-none shadow-sm rounded-2xl overflow-hidden">
          ...
          <CardContent className="p-0">
            {loading ? (
              <div className="p-8 text-center text-foreground/70 font-medium">불러오는 중...</div>
            ) : policies.length === 0 ? (
              <div className="p-12 text-center flex flex-col items-center justify-center space-y-4">
                <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-2">
                  <FileText className="w-8 h-8 text-foreground/70" />
                </div>
                <h3 className="text-lg font-bold text-obsidian">등록된 약관이 없습니다</h3>
                <p className="text-foreground/70 mb-4">기존의 하드코딩된 약관 텍스트들을 데이터베이스로 옮겨와 관리를 시작하세요.</p>
                <Button 
                  onClick={async () => {
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead className="bg-surface text-obsidian font-semibold border-b">
                    <tr>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {policies.map((p) => (
                      <tr key={p._id} className="hover:bg-surface/50 transition-colors">
                        <td className="px-6 py-4 font-mono text-foreground/70 font-bold">{p.type}</td>
                        <td className="px-6 py-4 font-bold text-obsidian">{p.title}</td>
                        <td className="px-6 py-4">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-black bg-slate-100 text-obsidian">
                        ...
                        <td className="px-6 py-4">
                          {p.isActive ? (
                             ...
                             <span className="inline-flex items-center text-foreground/70 font-bold text-xs">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end items-center gap-2">
                            <Button 
                              variant="secondary" 
                              size="sm" 
                              className="font-bold rounded-lg bg-gray-100 hover:bg-gray-200 text-obsidian"
                              asChild
                            >
