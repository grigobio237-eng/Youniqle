    ...
    return (
        <ChapterWrapper chapter="diagnosis-report">
            <div className="min-h-screen bg-[#F8FAFC]">
                {/* Navbar */}
                <div className="sticky top-0 z-[40] bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex items-center justify-between">
                ...
                <main className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-12 space-y-12 md:space-y-16" ref={reportRef}>
                    ...
                    <section className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                        ...
                        {/* Radar Chart Summary */}
                        <Card className="bg-white border-slate-100 shadow-lg rounded-[32px] md:rounded-[40px] overflow-hidden">
                            <CardContent className="p-0 grid grid-cols-1 md:grid-cols-[1.2fr_1fr]">
                                ...
                                <div className="p-6 md:p-12 bg-slate-50/30 border-t md:border-t-0 md:border-l border-slate-100 flex flex-col justify-center space-y-6">
                                    <div>
                    ...
                    <section className="space-y-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-slate-500 text-sm">성향 및 회복 요인에 기반한 당신의 고유한 기질입니다.</p>
                            </div>
                        </div>
                            ...
                            {hasAccess ? (
                                big5DomainData.length > 0 ? (
                                    big5DomainData.map((domain) => (
                                        <Card key={domain.subject} className="bg-white border-slate-100 shadow-sm hover:shadow-md transition-shadow rounded-[24px] md:rounded-[32px] overflow-hidden group">
                                            <CardContent className="p-0 flex flex-col md:flex-row">
                                                ...
                                                <div className="flex-1 p-5 md:p-8">
                                                    <div className="flex items-center gap-2 mb-3 md:mb-4">
                                                        ...
                                                        <span className="text-xs md:text-sm font-bold text-slate-800">분석 인사이트</span>
                                                    </div>
                                                    <p className="text-slate-600 text-xs md:text-sm leading-relaxed">{getInterpretation(domain.subject, domain.score)}</p>
                                                    <div className="mt-4 md:mt-6">
                                                        <div className="flex justify-between items-center mb-1.5 md:mb-2">
                                                            <span className="text-[10px] md:text-xs font-bold text-slate-400">강도</span>
                                                            <span className="text-[10px] md:text-xs font-bold text-slate-600">{domain.score}/100</span>
                                                        </div>
                                    ...
                                    <Card className="border-dashed border-2 border-slate-200 bg-slate-50/50 p-12 text-center rounded-[32px]">
                                        <p className="text-slate-500 font-medium">상세 분석 데이터가 없습니다. 정밀 진단을 진행해 보세요.</p>
                    ...
                    {hasAccess && (
                        <section className="space-y-8">
                            <div className="flex items-center gap-4">
                                ...
                                <div><h3 className="text-2xl font-black text-obsidian">세부 분석 국면</h3><p className="text-slate-500 text-sm">나를 설명하는 디테일한 키워드들입니다.</p></div>
                            </div>
                            ...
                            {big5DomainData.some(d => d.facets.length > 0) ? (
                                                    ...
                                                    return (
                                                        <div key={facet.id} className="group">
                                                            <div className="flex justify-between items-end mb-2">
                                                                <span className="font-bold text-slate-700 text-sm group-hover:text-obsidian transition-colors">{facet.name}</span>
                                ...
                                <Card className="border-dashed border-2 border-slate-200 bg-slate-50/50 p-12 text-center rounded-[32px]">
                                    <p className="text-slate-500 font-medium">세부 국면 분석 데이터는 정밀 진단 시 제공됩니다.</p>
                                </Card>
                            )}
                        </section>
                    )}
                    ...
                    {hasAccess && (
                        <div className="space-y-8">
                            <div className="flex items-center gap-4">
                                ...
                                <div><h3 className="text-2xl font-black text-obsidian">유니클 맞춤 솔루션</h3><p className="text-slate-500 text-sm">진단 결과를 바탕으로 설계된 처방전입니다.</p></div>
                            </div>
                            <GoldenTimeOffer
