...
export default function RecoveryLeaderboard() {
    ...
    if (loading) return (
        <div className="h-64 flex items-center justify-center bg-white rounded-[32px] animate-pulse">
            <span className="text-slate-400 font-bold">리더보드 로드 중...</span>
        </div>
    );

    return (
        <Card className="border-none shadow-sm rounded-[32px] bg-white overflow-hidden border border-slate-100 h-full flex flex-col justify-between">
            <div className="p-6 md:p-8 flex-1 flex flex-col justify-between">
                <div>
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <div className="flex items-center gap-2 text-[10px] font-black text-slate bg-slate-50 px-3 py-1 rounded-full uppercase tracking-widest">
                            ...
                            leaderboard.map((user, idx) => (
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-[10px] font-bold text-emerald-600 uppercase">Avg {user.averageScore}%</p>
                                    </div>
                                </motion.div>
                            ))
                        ) : (
                            <div className="py-10 text-center">
                                <p className="text-sm font-bold text-slate-400">이번 주 기록된 리듬이 아직 없습니다.</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="mt-8 space-y-4">
                    {leaderboard.length < 4 && (
                        <div className="p-4 rounded-2xl bg-emerald-50/40 border border-emerald-100/50 flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] font-semibold text-emerald-600 mt-1 leading-relaxed">
                    ...
                    <div className="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100">
                        <p className="text-[10px] leading-relaxed text-indigo-700 font-bold">
