...
function ToolkitItem({ label, desc, icon, href, color, isLocked }: ToolkitItemProps) {
    return (
        <Link href={isLocked ? '#' : href} className={`group ${isLocked ? 'cursor-not-allowed opacity-60' : ''}`}>
            <div className="bg-white border border-line rounded-[32px] p-8 hover:shadow-2xl transition-all flex flex-col items-center text-center h-full relative overflow-hidden group-hover:border-chapter-accent/30">
                <div className="w-20 h-20 bg-mist rounded-[24px] mb-6 flex items-center justify-center text-4xl group-hover:scale-110 transition-transform shadow-inner relative">
                ...
                <h3 className="text-xl font-black text-obsidian mb-2">{label}</h3>
                <p className="text-xs text-slate font-medium mb-6 leading-relaxed opacity-60 px-4">{desc}</p>
    ...
    return (
        <div className="space-y-12 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <ToolkitItem 
                    label="자세 밸런스 분석" 
                    desc="유니클 비전 기술을 활용한 실시간 체형 및 균형 측정" 
                    icon="🧘" 
            ...
            {/* Action Card */}
            <Card className="bg-obsidian text-white rounded-[40px] overflow-hidden border-none shadow-2xl relative">
                ...
                <CardContent className="p-10 relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                    ...
                    <Link href="/diagnosis?type=daily" className="bg-white text-obsidian px-10 py-5 rounded-2xl font-black text-lg hover:bg-white/90 hover:scale-105 transition-all shadow-xl shadow-white/10 whitespace-nowrap">
                        지금 분석 시작하기
                    </Link>
                </CardContent>
            </Card>
