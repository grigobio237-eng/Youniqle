...
export default function GuardianNudgeCard({ playerName, fatigueLevel }: GuardianNudgeCardProps) {
  const getNudgeMessage = () => {
    switch (fatigueLevel) {
      case 'HIGH':
        return `오늘 ${playerName} 선수의 피로도가 평소보다 높게 측정되었습니다. 훈련 후 "오늘 무릎은 괜찮았어?"라고 가볍게 물어봐 주시면 아이에게 큰 힘이 됩니다.`;
      case 'MEDIUM':
        return `${playerName} 선수가 이번 주 무난하게 훈련을 소화하고 있습니다. "오늘 수고했어!"라는 따뜻한 격려 한마디 부탁드립니다.`;
      case 'LOW':
        return `현재 ${playerName} 선수의 회복 상태가 매우 좋습니다. 주말 경기를 대비해 컨디션을 잘 유지할 수 있도록 응원해 주세요!`;
      default:
        return `오늘도 ${playerName} 선수를 응원해 주세요!`;
    }
  };
