                ...
                <CardContent className="px-0">
                    {loading ? (
                        ...
                        <div className="text-center py-12 border-2 border-dashed border-slate-100 rounded-xl">
                            ...
                            <p className="text-slate-400 font-medium text-sm">접수된 문의가 없습니다.</p>
                        </div>
                    ) : (
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {inquiries.map((inquiry) => (
                                        <TableRow key={inquiry._id} className="group hover:bg-slate-50 border-slate-50">
                                            <TableCell className="font-mono text-xs text-slate-400">
                                                {inquiry.inquiryId}
                                            </TableCell>
                                            <TableCell>
                                                <div>
                                                    <p className="font-bold text-sm text-slate-700">{inquiry.userName}</p>
                                                    <p className="text-[10px] text-slate-400">{inquiry.userEmail}</p>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="font-medium text-sm truncate max-w-[200px] text-slate-700">
                                                        {inquiry.subject}
                                                    </p>
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-right">
                                                ...
                                                >
                                                    <Eye className="h-4 w-4 text-slate-400 group-hover:text-slate-700" />
                                                </Button>
                                            </TableCell>
                                        </TableRow>
                                    ))}
            ...
            <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
                <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto p-0 gap-0 overflow-hidden bg-white rounded-2xl">
                    <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                        ...
                        <Badge variant="outline" className="bg-white font-mono text-slate-400">
                        ...
                        <div className="flex flex-col md:flex-row h-[70vh]">
                            ...
                            <div className="flex-1 p-6 overflow-y-auto border-r border-slate-100 space-y-8">
                                ...
                                <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                                    ...
                                    <div>
                                        <p className="font-bold text-slate-900">{selectedInquiry.userName}</p>
                                        <p className="text-xs text-slate-500">{selectedInquiry.userEmail}</p>
                                    </div>
                                    <div className="ml-auto text-right">
                                        <p className="text-xs font-bold text-slate-400">작성일</p>
                                        <p className="text-xs text-slate-500">{new Date(selectedInquiry.createdAt).toLocaleString('ko-KR')}</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-bold text-slate-900 mb-2">{selectedInquiry.subject}</h3>
                                        <div className="p-6 bg-slate-50/50 rounded-2xl border border-slate-100 text-sm leading-relaxed text-slate-700 whitespace-pre-wrap">
                                ...
                                {selectedInquiry.attachments && selectedInquiry.attachments.length > 0 && (
                                    <div className="space-y-3">
                                        <h5 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Attachments</h5>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            {selectedInquiry.attachments.map((file, index) => (
                                                <a
                                                    ...
                                                    className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 bg-white hover:border-indigo-200 hover:bg-indigo-50/50 transition-all group"
                                                >
                                                    <div className="p-2 bg-slate-100 rounded-lg group-hover:bg-white transition-colors">
                                                        <Paperclip className="h-4 w-4 text-slate-400 group-hover:text-indigo-500" />
                                                    </div>
                            ...
                            <div className="w-full md:w-[400px] bg-slate-50 flex flex-col">
                                <div className="p-4 border-b border-slate-200/60 bg-white/50 backdrop-blur-sm sticky top-0">
                                ...
                                <div className="flex-1 p-4 overflow-y-auto space-y-6">
                                    ...
                                    {selectedInquiry.aiAnswer && (
                                        <div className="space-y-2">
                                            ...
                                            <Button size="sm" variant="ghost" className="w-full text-xs text-purple-600 h-8 hover:bg-purple-100" onClick={() => setAdminAnswer(selectedInquiry.aiAnswer || '')}>
                                                이 답변 사용하기
                                            </Button>
                                        </div>
                                    )}
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-slate-500 uppercase">관리자 답변 작성</label>
                                        <Textarea
                                            value={adminAnswer}
                                            onChange={(e) => setAdminAnswer(e.target.value)}
                                            placeholder="고객에게 전송될 답변을 입력하세요..."
                                            className="min-h-[200px] resize-none p-4 text-sm bg-white border-slate-200 focus:border-indigo-500 focus:ring-indigo-500/20"
                                        />
                                    </div>
                                </div>
