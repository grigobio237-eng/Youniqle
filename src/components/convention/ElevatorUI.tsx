...
export default function ElevatorUI({ activeFloor, onFloorChange }: ElevatorUIProps) {
    ...
    return (
        <div className="fixed left-10 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-6">
            ...
            {floors.map((floor) => (
                ...
                >
                    ...
                    <div className={`w-3 h-3 rounded-full border-2 transition-all duration-300 ${activeFloor === floor.id ? 'bg-[#D4AF37] border-[#D4AF37] scale-125 shadow-lg shadow-[#D4AF37]/30' : `${borderColor} group-hover:border-white/20`}`} />
            ...
            <div className="mt-8 text-center">
                <p className={`text-[10px] font-black ${isDarkTheme ? 'text-[#D4AF37]/30' : 'text-[#D4AF37]/40'} uppercase tracking-[0.3em] vertical-text`}>
                    Level Navigator
                </p>
            </div>
        </div>
