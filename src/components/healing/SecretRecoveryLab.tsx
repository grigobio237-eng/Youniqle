...
export default function SecretRecoveryLab() {
    ...
    if (step === 'MAIN') {
        return (
            <div className="w-full flex flex-col items-center justify-center py-20 bg-[#F9F7F2]">
    ...
    if (step === 'FORM') {
        return (
            <div className="w-full flex justify-center py-20 bg-[#F9F7F2]">
                <Card className="max-w-2xl w-full mx-6 p-10 md:p-16 border-none shadow-[0_40px_100px_rgba(0,0,0,0.08)] rounded-[48px] bg-white relative overflow-hidden">
                        ...
                        {formStep === 1 && (
                            ...
                            >
                                <div className="space-y-4">
                                    ...
                                    <h2 className="text-4xl font-black text-[#0B0D10] tracking-tighter leading-[1.1]">
                                        당신이 지금 <br /> 가장 해결하고 싶은 <br /> 고민은 무엇인가요?
                                    </h2>
                                </div>
                                <RadioGroup value={formData.goal} onValueChange={(v) => setFormData({ ...formData, goal: v })} className="grid grid-cols-1 gap-4">
                                    {[
                                        ...
                                        <div key={item.id} className={`flex items-center space-x-4 border-2 p-6 rounded-[24px] cursor-pointer transition-all ${formData.goal === item.id ? 'border-[#D4AF37] bg-[#D4AF37]/5' : 'border-[#0B0D10]/5'}`} onClick={() => setFormData({ ...formData, goal: item.id })}>
                                            <div className={`w-3 h-3 rounded-full ${formData.goal === item.id ? 'bg-[#D4AF37]' : 'bg-[#0B0D10]/10'}`} />
                                            <Label className="text-lg font-bold text-[#0B0D10] cursor-pointer flex-1">{item.label}</Label>
                                        </div>
                                    ))}
                                </RadioGroup>
                                <Button className="w-full h-16 bg-[#0B0D10] text-white rounded-[20px] font-black text-sm uppercase tracking-widest disabled:opacity-20" disabled={!formData.goal} onClick={handleFormNext}>다음 단계로</Button>
                            </motion.div>
                        )}
                        ...
                        {formStep === 2 && (
                            ...
                            >
                                ...
                                <div className="space-y-8">
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-end">
                                            <Label className="text-sm font-black text-[#0B0D10] uppercase tracking-widest opacity-40">평균 스트레스 지수</Label>
                                            <span className="text-2xl font-black text-[#0B0D10]">{formData.stress}%</span>
                                        </div>
                                        <input 
                                            type="range" 
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-end">
                                            <Label className="text-sm font-black text-[#0B0D10] uppercase tracking-widest opacity-40">평균 수면 시간</Label>
                                            <span className="text-2xl font-black text-[#0B0D10]">{formData.sleep}시간</span>
                                        </div>
                                        <input 
                                            type="range" 
                                            min="3" 
                                ...
                                <div className="flex gap-4">
                                    <Button variant="ghost" className="h-16 flex-1 rounded-[20px] font-black text-[#0B0D10]/30 border border-[#0B0D10]/5" onClick={() => setFormStep(1)}>이전</Button>
                                    <Button className="h-16 flex-[2] bg-[#0B0D10] text-white rounded-[20px] font-black text-sm uppercase tracking-widest" onClick={handleFormNext}>다음 단계로</Button>
                                </div>
                            </motion.div>
                        )}
                        ...
                        {formStep === 3 && (
                            ...
                            >
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-4">
                                        <Label className="text-sm font-black text-[#0B0D10] uppercase tracking-widest opacity-40">월 가용 예산</Label>
                                        <RadioGroup value={formData.budget} onValueChange={(v) => setFormData({ ...formData, budget: v })} className="grid grid-cols-2 gap-3">
                                            {[
                                                ...
                                                <div key={b.id} className={`p-4 border-2 rounded-2xl cursor-pointer text-center font-bold text-sm transition-all ${formData.budget === b.id ? 'border-[#D4AF37] bg-[#D4AF37]/5 text-[#D4AF37]' : 'border-[#0B0D10]/5 text-[#0B0D10]/40'}`} onClick={() => setFormData({ ...formData, budget: b.id })}>
                                                    {b.label}
                                                </div>
                                            ))}
                                        </RadioGroup>
                                    ...
                                    <div className="space-y-4">
                                        <Label className="text-sm font-black text-[#0B0D10] uppercase tracking-widest opacity-40">특이 사항 (알러지, 이전 병력 등)</Label>
                                        <Input placeholder="정밀 설계에 필요한 추가 정보를 알려주세요." className="h-16 rounded-[20px] bg-[#F9F7F2] border-none px-6" value={formData.history} onChange={(e) => setFormData({ ...formData, history: e.target.value })} />
                                    </div>
                                </div>
                                <div className="flex gap-4">
    ...
    if (step === 'RESULT') {
        return (
            <div className="w-full py-20 bg-[#F9F7F2] px-6">
                <div className="max-w-6xl mx-auto space-y-20">
                    <div className="text-center space-y-6">
                        <Badge className="bg-[#D4AF37]/10 text-[#D4AF37] border-none px-4 py-1.5 font-black uppercase tracking-widest text-[10px]">Analysis Result</Badge>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {Object.entries(youniqlePlans.plans).map(([key, plan]) => (
                            <Card 
                                ...
                                className={`group cursor-pointer transition-all duration-500 rounded-[40px] overflow-hidden flex flex-col border-4 ${selectedPlan === plan.planId ? 'border-[#D4AF37] bg-white shadow-2xl scale-[1.02]' : 'border-transparent bg-white/50 hover:border-[#D4AF37]/20 shadow-sm'}`}
                            >
                                <div className={`p-10 space-y-6 ${key === 'planC' ? 'bg-[#D4AF37]/5' : ''}`}>
                                    ...
                                    <h3 className="text-3xl font-black text-[#0B0D10] leading-tight tracking-tighter">{plan.title}</h3>
                                ...
                                <CardContent className="p-10 space-y-10 flex-1 flex flex-col">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-[#F9F7F2] rounded-2xl p-4">
                                            <p className="text-[9px] font-black text-[#0B0D10]/20 uppercase tracking-widest mb-1">Estimate</p>
                                            <p className="text-sm font-black text-[#0B0D10] tracking-tight">{plan.priceEstimate}</p>
                                        </div>
                                        <div className="bg-[#F9F7F2] rounded-2xl p-4">
                                            <p className="text-[9px] font-black text-[#0B0D10]/20 uppercase tracking-widest mb-1">Focus</p>
                                            <p className="text-sm font-black text-[#0B0D10] tracking-tight">{plan.focusArea}</p>
                                        </div>
                                    </div>
                                        ...
                                        <ul className="space-y-4">
                                            {plan.routine.map((item, i) => (
                                                <li key={i} className="flex items-start gap-3">
                                                    ...
                                                    <span className="text-sm font-bold text-[#0B0D10]/70 leading-tight">{item}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>

                                    <div className={`mt-auto pt-8 flex justify-center transition-all ${selectedPlan === plan.planId ? 'scale-100 opacity-100' : 'scale-50 opacity-0'}`}>
                                        <div className="w-12 h-12 rounded-full bg-[#0B0D10] text-white flex items-center justify-center shadow-lg">
                    ...
                    <div className="flex flex-col items-center gap-6">
                        <Button 
                            ...
                            className="h-20 px-20 bg-[#0B0D10] text-white rounded-[24px] font-black text-xl uppercase tracking-widest shadow-2xl transition-all hover:scale-105 disabled:opacity-20"
                        >
                            컨시어지 상담 신청하기
                        </Button>
                        <p className="text-[10px] font-black text-[#0B0D10]/30 uppercase tracking-[0.3em]">* 최종 상담 후 프로그램이 확정됩니다.</p>
                    </div>
                </div>
            </div>
        );
    ...
    if (step === 'SUBMITTED') {
        return (
            <div className="w-full h-[600px] flex flex-col items-center justify-center text-center p-6 bg-[#F9F7F2]">
                <div className="w-24 h-24 bg-white rounded-[40px] flex items-center justify-center shadow-2xl mb-10 border-4 border-[#D4AF37]/20">
                    <ShieldCheck className="w-12 h-12 text-[#D4AF37]" />
                </div>
                <div className="space-y-6">
                    <h2 className="text-4xl md:text-6xl font-black text-[#0B0D10] tracking-tighter leading-none italic">
                        Protocol <span className="text-[#D4AF37] tracking-normal">Accepted.</span>
                    </h2>
                    <p className="text-xl text-[#0B0D10]/40 font-medium max-w-xl mx-auto leading-relaxed">
                        신청이 성공적으로 접수되었습니다. <br />
                        24시간 이내에 원장 전담 팀에서 <br />
                        <span className="text-[#D4AF37]">당신만을 위한 초대장</span>을 발송합니다.
                    </p>
                </div>
                <Button onClick={() => setStep('MAIN')} variant="ghost" className="mt-12 font-black text-[10px] uppercase tracking-widest opacity-30 hover:opacity-100 transition-all">설계 다시 하기</Button>
            </div>
        );
    }
