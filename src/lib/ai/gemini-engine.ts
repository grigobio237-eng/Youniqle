...
- **5층 - 프라이빗 라운지 (/lounge)**: 김미정 원장과의 1:1 AI 심층 상담 (Founder Pass 또는 구독 회원 전용)

### 🎨 AI 웹툰 챌린지
- 사용자의 회복 데이터를 기반으로 AI가 4컷 웹툰을 생성해주는 서비스
- 생성된 웹툰은 SNS(인스타그램 등) 공유용 캡션과 함께 제공됨

### 💎 Founder Pass (파운더스 패스)
유니클 생태계의 핵심 입장권이자 프리미엄 멤버십입니다. (/founder-pass)
1. **Tier 1: ESSENCE (39만원)** - 디지털 스타터. 온라인 생체 나이 진단, 기초 주간 리포트, 스타터 키트 제공. 상시 5% 할인.
2. **Tier 2: BALANCE (69만원)** - 테크 엔수지아스트. 8대 무형 자산 VOD 강좌, 3D 데이터 갤러리, AI 내비게이터 플러스. 상시 10% 할인.
3. **Tier 3: MIRACLE (99만원)** - 파워 유저. 월 1회 AI 웹툰 생성, 그룹 멘탈 큐레이팅 웹세미나, 보안 디지털 금고. 상시 15% 할인.
* **관계**: Founder Pass는 기존 멤버십의 상위 등급이며, 가입 시 모든 프리미엄 서비스 이용이 가능하여 별도의 월 구독 멤버십이 필요하지 않습니다.

### 📊 등급 및 멤버십 시스템
- **Founder Pass 계열**: 최고의 권한을 가진 프리미엄 등급 (ESSENCE/BALANCE/MIRACLE)
